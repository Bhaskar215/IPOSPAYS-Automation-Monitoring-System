...
public class QPaySendLink_TipTX_CustomFeePage extends BasePage {
    ...
    private final By tipSuggestion1Field = By.xpath("//*[contains(text(),'Suggestions')]//following::input[1]");
...
*/
    ...
    public void setTipSuggestion(Map<String,String>data){
        sendKeys(tipSuggestion1Field,data.get("Tip Suggestions1"),WaitStrategy.VISIABLE);
    }


    public void verifyTipSetToYes(Map<String,String>data) throws InterruptedException {

        //If Tip Set already No , then don't need to FindElement ,
        // If Tip Set to Yes , then we need to FindElement ,and Set to YES.

        String IsTipsetToYes =DynamicXpathUtils.getXpath(CustomXpath.isBtnDisable,"Tip","Yes");
        System.out.println(IsTipsetToYes);
        boolean isTipEnable =StringUtil.isBtnDisable(By.xpath(IsTipsetToYes));
        System.out.println(isTipEnable);
        if(isTipEnable) {
            System.out.println("Already Tip Enable");
        }
        else{
            Thread.sleep(1000);
    ...
    public QPaySendLink_TipTX_CustomFeePage goToInvoiceModule(){
        click(clickInvoices,WaitStrategy.VISIABLE);
        return this;
    }

    public QPaySendLink_TipTX_CustomFeePage searchReferenceNumber(){
        sendKeys(searchRef,getReferenceNum(),WaitStrategy.VISIABLE);
        return this;
    }

    public QPaySendLink_TipTX_CustomFeePage clickTheRefreshBtn(){
    /*    try {
            Thread.sleep(1000);
        } catch (InterruptedException e) {
            e.printStackTrace();
     ...
     *
     */
    public QPaySendLink_TipTX_CustomFeePage verifyInvoiceStatusBeforeTx(){
    ...
    public QPaySendLink_TipTX_CustomFeePage verifyInvoiceStatusAfterTx() throws InterruptedException {
        Thread.sleep(2000);
        String isStatuspendingTxt = getTexts(getStatus,WaitStrategy.VISIABLE);
    ...
    public QPaySendLink_TipTX_CustomFeePage switchToParentWindow(WebDriver driver) {
